...
func newSysMenu(db *gorm.DB, opts ...gen.DOOption) sysMenu {
	...
	_sysMenu.HideInMenu = field.NewBool(tableName, "hide_in_menu")
	_sysMenu.IgnoreKeepAlive = field.NewBool(tableName, "ignore_keep_alive")
	_sysMenu.LinkFlag = field.NewBool(tableName, "link_flag")
	_sysMenu.Link = field.NewString(tableName, "link")
	_sysMenu.Disabled = field.NewBool(tableName, "disabled")
...
type sysMenu struct {
	...
	Link            field.String // 跳转路径 （外链）
	Disabled        field.Bool   // 是否停用 0 不停用 1 启用，对应 menuVisibleWithForbidden
...
func (s *sysMenu) updateTableName(table string) *sysMenu {
	...
	s.HideInMenu = field.NewBool(table, "hide_in_menu")
	s.IgnoreKeepAlive = field.NewBool(table, "ignore_keep_alive")
	s.LinkFlag = field.NewBool(table, "link_flag")
...
func (s *sysMenu) fillFieldMap() {
	...
	s.fieldMap["ignore_keep_alive"] = s.IgnoreKeepAlive
	s.fieldMap["link_flag"] = s.LinkFlag
