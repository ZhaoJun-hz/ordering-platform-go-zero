...
func newSysToken(db *gorm.DB, opts ...gen.DOOption) sysToken {
	...
	_sysToken.ID = field.NewInt64(tableName, "id")
	_sysToken.Status = field.NewInt32(tableName, "status")
	_sysToken.UserID = field.NewInt64(tableName, "userId")
	_sysToken.Username = field.NewString(tableName, "username")
	_sysToken.Token = field.NewString(tableName, "token")
	_sysToken.ExpiredAt = field.NewInt64(tableName, "expired_at")
	_sysToken.CreatedAt = field.NewTime(tableName, "created_at")
	_sysToken.UpdatedAt = field.NewTime(tableName, "updated_at")
...
type sysToken struct {
	...
	Token     field.String // Token 字符串
	ExpiredAt field.Int64  // 过期时间
	CreatedAt field.Time   // 创建日期
	UpdatedAt field.Time   // 修改日期
...
func (s *sysToken) WithContext(ctx context.Context) ISysTokenDo { return s.sysTokenDo.WithContext(ctx) }

func (s sysToken) TableName() string { return s.sysTokenDo.TableName() }

func (s sysToken) Alias() string { return s.sysTokenDo.Alias() }

func (s sysToken) Columns(cols ...field.Expr) gen.Columns { return s.sysTokenDo.Columns(cols...) }
